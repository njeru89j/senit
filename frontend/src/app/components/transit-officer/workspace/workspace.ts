...
export class TransitWorkspace implements OnInit {
  ...
  selected: any = null; inspections: Record<string, { condition: string; notes: string }> = {}; busy = false; message = ''; tab = 'batches';
  handoff = { routeId: '', transitPointId: '', parcelIds: [] as string[] };
  ...
  open(batch: any) { this.selected = batch; this.inspections = {}; for (const parcel of batch.parcels) this.inspections[parcel.id] = { condition: '', notes: '' }; }
  ...
  dismiss() {
    ...
    this.api.inspectAndDismissBatch(this.selected.id, data).subscribe({ next: (result: any) => { this.busy = false; this.message = `Batch dismissed: ${result.dismissed} cleared, ${result.held} held for review.`; this.selected = null; this.load(); }, error: (e: any) => { this.busy = false; this.message = e.error?.message ?? 'Inspection could not be completed'; } });
  }
  ...
  handOff() {
    if (!this.handoff.routeId || !this.handoff.transitPointId || !this.handoff.parcelIds.length) return;
    this.busy = true; this.message = '';
    this.api.verifyParcelsAtTransit(this.handoff).subscribe({ next: () => this.api.createBatch({ routeId: this.handoff.routeId, parcelIds: this.handoff.parcelIds }).subscribe({ next: () => { this.busy = false; this.message = 'Handoff recorded and onward batch created. The driver can load and depart it.'; this.handoff.parcelIds = []; this.load(); }, error: (e: any) => { this.busy = false; this.message = e.error?.message ?? 'Parcel verified, but batch creation failed'; } }), error: (e: any) => { this.busy = false; this.message = e.error?.message ?? 'Could not verify the parcel handoff'; } });
  }
  assignLocker() {
